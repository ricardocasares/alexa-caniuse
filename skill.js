"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = y[op[0] & 2 ? "return" : op[0] ? "throw" : "next"]) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [0, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
exports.__esModule = true;
var api = require("caniuse-api");
var Alexa = require("ask-sdk-core");
var LaunchRequest = {
    canHandle: function (input) {
        var type = input.requestEnvelope.request.type;
        return type === "LaunchRequest";
    },
    handle: function (input) {
        return input.responseBuilder
            .speak("Welcome to Can I Use")
            .reprompt("Ask me about a CSS property")
            .getResponse();
    }
};
var CanIUseRequest = {
    canHandle: function (input) {
        var type = input.requestEnvelope.request.type;
        var intent = input.requestEnvelope.request.intent;
        return type === "IntentRequest" && intent.name === "CanIUse";
    },
    handle: function (input) {
        var request = input.requestEnvelope.request;
        var slots = request.intent.slots;
        var feature = slots.feature;
        if (!feature) {
            return input.responseBuilder
                .speak("Please tell me a feature")
                .reprompt("tell me a feature")
                .getResponse();
        }
        var support = api.getSupport(toSlotID(feature));
        return input.responseBuilder
            .speak("Welcome to Can I Use")
            .reprompt("Ask me about a CSS property")
            .getResponse();
    }
};
var ErrorHandler = {
    canHandle: function (input) {
        return true;
    },
    handle: function (input) {
        return input.responseBuilder.getResponse();
    }
};
exports.handler = function (event) { return __awaiter(_this, void 0, void 0, function () {
    return __generator(this, function (_a) {
        return [2 /*return*/, Alexa.SkillBuilders.custom()
                .addRequestHandlers(LaunchRequest, CanIUseRequest)
                .addErrorHandlers(ErrorHandler)
                .lambda()];
    });
}); };
function toSlotID(slot) {
    // THIS IS INSECURE @TODO: MAKE CHECKS FOR MISSING STUFF AND FAILOVERS
    var name = slot.name, resolutions = slot.resolutions;
    var resolutionsPerAuthority = resolutions.resolutionsPerAuthority;
    var resolution = resolutionsPerAuthority[0];
    var values = resolution.values, code = resolution.status.code;
    var first = values[0];
    return { name: name, id: first.id };
}
function humanReadable(support) {
    return Object.keys(support)
        .map(function (browser) {
        var vers = support[browser];
        if (!vers.y)
            return browser + " is not supported.";
        return browser + " since version " + vers.y;
    })
        .join("\n");
}
